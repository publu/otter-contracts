...
describe('Bonding', () => {
  ...
  beforeEach(async () => {
    ...
    await qi.approve(daiBonx.address, "10000000000000000000000000000000");

    await daiBond.fund("10000000000000000000")
  ...
  describe('deposit', () => {
    it('should get vested fully', async () => {
      ...
      await expect(() =>
        daiBond.redeem(deployer.address)
      ).to.changeTokenBalance(qi, deployer, '30834236186')
    })
    ...
    it('should get vested partially', async () => {
      ...
      await expect(() =>
        daiBond.redeem(deployer.address)
      ).to.changeTokenBalance(qi, deployer, totalClam.div(5))

      // fully vested, get rest 80%
      await timeAndMine.setTimeIncrease(10)
      await expect(() =>
        daiBond.redeem(deployer.address)
      ).to.changeTokenBalance(qi, deployer, totalClam - totalClam.div(5))
    })
